 ...
 */
bool DatabaseHelper::createNewUserMessage(qint32 userSenderId, qint32 userReceiverId, QString strMessage)
{
    // exit if something is wrong
    if(userSenderId == 0 || userReceiverId == 0 || strMessage.isEmpty()) {
        return false;
    }

    // build message query
    QString strQueryMessage = QString(
                "INSERT INTO usermessage(sender_user_id, receiver_user_id, message, created) "
                "VALUES(%1, %2, '%3', NOW());"
    ).arg(userSenderId).arg(userReceiverId).arg(strMessage);

    // create new user message
    return QSqlQuery(strQueryMessage, QSqlDatabase::database()).exec();
}
